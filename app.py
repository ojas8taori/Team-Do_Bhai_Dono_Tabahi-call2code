...
from components.voice_features import render_voice_features
from components.multi_user_collaboration import render_multi_user_collaboration
...
def load_css():
    ...
    if st.session_state.get('dark_mode', False):
        css_content += """
        .stApp {
            background: linear-gradient(135deg, #1a1d29, #2d3748) !important;
            color: #ffffff !important;
        }
        
        .stSidebar {
            background: linear-gradient(180deg, #2d3748, #4a5568) !important;
        ...
        /* Navigation menu styling for dark mode - ORANGE BACKGROUND */
        .nav-link {
            color: #ffffff !important;
        ...
        /* Option menu container for dark mode - ORANGE */
        .css-1d391kg {
            background: #ff6b35 !important;
        ...
        .stApp *, .stSidebar *, .stMarkdown *, .stText *, .stCaption *,
        ...
        /* FORCE ALL TEXT BLUE IN LIGHT MODE */
        .stApp *, .stSidebar *, .stMarkdown *, .stText *, .stCaption *,
        .stMetric *, .stButton *, .stSelectbox *, .stTextInput *,
        div, span, p, h1, h2, h3, h4, h5, h6, label {
    ...
    # Add market change color coding and navigation menu styling
    css_content += """
    /* Navigation menu styling for light mode - ORANGE BACKGROUND */
    .nav-link {
        color: #ffffff !important;
        font-weight: 500 !important;
        background: #ff6b35 !important;
    ...
    /* Option menu container for light mode - ORANGE */
    .css-1d391kg {
        background: #ff6b35 !important;
        border-radius: 10px !important;
        padding: 0.5rem !important;
    ...
    /* Metric delta styling - FORCE GREEN/RED COLORS */
    ...
    .stMetric [data-testid="metric-delta"][data-direction="positive"] {
        color: #10b981 !important;
    }
    
    /* Red for negative values */
    .stMetric [data-testid="metric-delta"][data-direction="negative"] {
        color: #ef4444 !important;
    }
    
    /* Force positive/negative colors on metric values */
    .stMetric [data-testid="metric-delta"]::before {
        content: "";
    }
    ...
    /* Force color on arrow and percentage text */
    .stMetric [data-testid="metric-delta"] svg path {
...
def render_sidebar():
    with st.sidebar:
        ...
        if st.session_state.voice_enabled:
            st.info("🎙️ Voice features are now available in the 'Voice Features' tab above!")
...
def main():
    ...
    selected = option_menu(
        menu_title=None,
        options=["Dashboard", "Stock Analysis", "Market Overview", "News Feed", "Story Mode", "Voice Features", "Multi-User"],
        icons=["speedometer2", "graph-up", "globe", "newspaper", "book", "mic", "people"],
        menu_icon="cast",
        default_index=default_index,
        orientation="horizontal",
        styles={
            "container": {"padding": "0!important", "background-color": "#FF6B35"},
            "icon": {"color": "#00FF00", "font-size": "18px"},
            "nav-link": {
                "font-size": "16px",
                "text-align": "center",
                "margin": "0px",
    ...
    try:
        if selected == "Dashboard":
            ...
            render_story_mode()
        elif selected == "Voice Features":
            render_voice_features()
        elif selected == "Multi-User":
            render_multi_user_collaboration()
